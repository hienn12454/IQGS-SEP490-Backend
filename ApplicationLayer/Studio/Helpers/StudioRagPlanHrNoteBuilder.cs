...
public static class StudioRagPlanHrNoteBuilder
{
    ...
    public static string BuildInitial(Guid projectId, int selectedDocs, int totalQuestions, int interviewMinutes)
    {
        ...
        sb.AppendLine(
            "- question_type_distribution: đủ 4 loại technical, system_design, problem_solving, behavioral — mỗi loại count>0, reason tiếng Việt.");
        sb.AppendLine(
            "- recommended_question_outline: đúng số câu; mỗi item có type/difficulty/skill/focus_area/goal rõ (goal tiếng Việt).");
        sb.AppendLine(
            "- coverage: ≥4 skill từ JD+RAG, mỗi skill có question_count; summary 2–3 câu mô tả cấu trúc buổi PV.");
        sb.AppendLine(
            "- Mỗi coverage PHẢI có source_files = tên file RAG thật từ retrieve (để UI hiện focus từ tài liệu nào).");
        sb.AppendLine(
            "- Section UI mong đợi: Technical Foundations, System Design, Problem Solving, Behavioral (map từ type).");
